...
async def _json_payload_from_request(request: Request) -> dict:
    ...
    if not isinstance(parsed, dict):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="JSON payload must be an object.")
    return dict(parsed)
...
def dual_bot_case_context(case_id: str, api_key: None = Depends(require_api_key)):
    try:
...
def dual_bot_guardian_feedback(case_id: str, payload: GuardianFeedbackRequest, api_key: None = Depends(require_api_key)):
    try:
