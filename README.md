...
Além do backend/frontend web, o repositório já inclui um multi-tenant inicial (`Organization`), pareamento de dispositivo com push notifications (FCM), um app **Android Companion** (Kotlin + Jetpack Compose, para o contato de confiança) e uma casca **Windows Desktop** (Tauri) — ver [Aplicativos Companion](#aplicativos-companion-android-e-windows) abaixo.

Detecção em camadas: regras determinísticas explicáveis (decidem hoje) + **pipeline híbrido com LLM e Policy Engine** já validado contra um LLM real e uma instância Evolution/WhatsApp real (desligado e em shadow por padrão — a LLM nunca decide envio; a decisão híbrida ainda **não está conectada** à criação de caso/notificação, ver Roadmap). **Regra cardinal: o bot nunca responde ao remetente.** O único destino possível de mensagem de saída é o contato de confiança cadastrado, e apenas em golpe explícito, sempre atrás do safety gate (simulação/envio real/allowlist).

O núcleo é open-source; uma versão cloud gerenciada faz parte da visão de longo prazo do projeto.
...
| Multi-tenant (`Organization`) | Implementado (base) | `organization_id` em `AuthUser`/`ProtectedPerson`/`Guardian`/`Case`, hoje opcional/retrocompatível. **Gap conhecido:** os repositórios de `Case`/`ProtectedPerson`/`Guardian` ainda não filtram por `organization_id` nas consultas — só `devices`/`notifications` já aplicam isolamento (404, não 403) e têm teste de IDOR entre orgs. Precisa fechar antes do multi-tenant pago. |
...
| Android Companion | Build validado, piloto em teste real | App Kotlin/Compose em `apps/android-companion/`; 17/17 testes unitários verdes; pareado e testado uma vez em aparelho físico real (ver `apps/android-companion/BUILD_NOTES.md`). Sem assinatura de release, sem UI própria de admin para gerar convite ainda. |
| Windows Desktop (Tauri) | Build validado localmente | Casca fina em `apps/windows-shell/` (sem backend local): exporta só Console/Admin/MFA/login do Next.js, chama a API via `@tauri-apps/plugin-http` (fora de CORS/cookie de navegador). Sem assinatura de código, sem updater configurado (ver `apps/windows-shell/BUILD_NOTES.md`). |
| Produção | Não pronta | Falta fechar IDOR multi-tenant acima, migrar persistência para Postgres (hoje só SQLite tem suporte real no código, mesmo com Postgres já no `compose.server.yml` para Evolution/n8n), observabilidade gerenciada, rate limit/idempotência distribuídos (hoje em memória por processo) e assinatura dos instaladores Windows/Android. |
...
- Fechar o isolamento multi-tenant (`Organization` já existe como base; falta filtrar `Case`/`ProtectedPerson`/`Guardian` por `organization_id`) e evoluir políticas refinadas por organização e gerenciamento completo de usuários.
...
## Arquitetura do Sistema (visão-alvo, com status real)

Arquitetura-alvo completa do Plano Mestre v1.1. Cada nó traz o status real apurado no repositório — não o que está no plano, mas o que foi de fato implementado e testado.

**Legenda:** ✅ implementado e testado · 🚧 implementado, com lacunas conhecidas ou validação parcial · 📋 planejado, ainda não implementado.
...
Estrutura de pastas real (backend/frontend/apps/docs) na seção [Estrutura de Pastas](#estrutura-de-pastas) abaixo.
...
Além do backend/frontend web, o produto tem dois clientes nativos que **não têm lógica de decisão própria** — a nuvem (FastAPI) continua sendo o cérebro; eles só autenticam, exibem o Console e recebem/confirmam alertas.
...
### Companion (Android e Windows)

- Android Companion: Kotlin, Jetpack Compose, Retrofit, Firebase Cloud Messaging (FCM).
- Windows Desktop: Tauri 2 (Rust) sobre o mesmo build Next.js exportado estaticamente.
...
│       ├── dual_bot/
│       ├── event_model/        # inclui Organization (multi-tenant)
│       ├── evolution_demo/
│       ├── guardian_console/
│       ├── hybrid/
│       ├── llm/
...
## Correções de Segurança Recentes

- Login Google OAuth/OIDC: corrigido bypass na validação do `nonce` (`backend/app/auth/google_oidc.py`) — um token do Google sem a claim `nonce` passava despercebido, pulando a defesa contra replay. Agora exige presença e correspondência do nonce sempre que um é esperado.
- Frontend: 20 chamadas de API (consentimento, Guardian Console, Trusted Circle, Proof of Trust em `frontend/lib/api.ts`) não enviavam `credentials: 'include'` — funcionavam por acidente hoje (front e backend na mesma origem local) e quebrariam quando front e backend estiverem em domínios separados na nuvem. Corrigido antes de virar um bug de produção.
- Auditoria confirmou: nenhum bug real encontrado no shell Windows (Tauri). Gap conhecido e ainda aberto (não é bug, é trabalho pendente): `Case`/`ProtectedPerson`/`Guardian` ainda não filtram por `organization_id` nos repositórios — só `devices`/`notifications` já isolam por organização com teste de IDOR. Ver Roadmap.
...
- **Conectar a decisão híbrida (LLM + Policy Engine) ao fluxo real de criação de caso/notificação** — hoje ela roda em paralelo, testada contra LLM real, e grava decisão auditável, mas quem decide se um caso é criado/notificado ainda é 100% o motor de regras determinísticas legado (achado da Sprint 5).
- Fechar o isolamento multi-tenant: filtrar `Case`/`ProtectedPerson`/`Guardian` por `organization_id` nos repositórios (hoje só `devices`/`notifications` já isolam com teste de IDOR).
- Migrar persistência de SQLite para Postgres em produção (código atual só suporta SQLite; Postgres já sobe no `compose.server.yml` para Evolution/n8n, mas o backend não usa).
- Implementar workers assíncronos (Celery) e tornar rate limit/idempotência distribuídos (Redis) — hoje em memória por processo, necessário para múltiplos workers/réplicas.
- Assinar os instaladores Windows (code signing) e Android (release key) antes de qualquer distribuição pública.
- Fechar lacunas de UI do Android Companion (lista de casos, UI de admin para convite) e do Windows Shell (botão de logout no console, deep link de caso específico).
- Consolidar persistência com migrações e retenção formal.
- Evoluir autenticação para reset de senha, recovery codes e políticas por organização (papéis já existem: `ADMIN`/`ANALYST`/`VIEWER`/`TRUSTED_CONTACT`).
